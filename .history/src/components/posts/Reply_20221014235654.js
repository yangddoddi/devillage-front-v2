...
export const Reply = ({
  ...
  console.log(reComment.map((re) => re.map((re) => re.commentId === reply.id)));
  ...
  return (
    <div className={styles.replyItem}>
      ...
      {reComment &&
        reComment.filter((item) =>
          item.filter((item) => item.parentCommentId === reply.id)
        ).length > 0 && (
          <div className={styles.replyOfCommentContainer}>
            {reComment
              .filter((item) => {
                console.log(item);
                return item.filter((item) => item.parentCommentId === reply.id);
              })
              .map((item) => (
                <ReplyOfComment
